...
| TD-006 | HIGH | `src/pages/*.astro`, `src/components/PricingSimulator.tsx`, `src/data/system-prompt.ts`, `src/layouts/BaseLayout.astro` | Tarifs et fiches projets dupliqués dans 4 sources sans référentiel commun. La divergence s'est déjà produite (TJM et statut Niido dans le prompt du chatbot contredisaient le site). Centraliser dans `src/data/offers.ts` et `src/data/projects.ts`, générer le prompt système à partir de ces données. | 2h | open |
| TD-007 | HIGH | `vercel.json` | La CSP autorise `script-src 'self' 'unsafe-inline'` alors que `BaseLayout.astro` charge Google Tag Manager depuis `googletagmanager.com` : le script est vraisemblablement bloqué en production. Vérifier la console sur l'environnement de prod, puis ajouter le domaine à `script-src`/`connect-src` ou retirer GTM. | 1h | open |
| TD-008 | MEDIUM | `src/styles/global.css`, `src/layouts/BaseLayout.astro` | Les classes `reveal-*` et `stagger-item` posent `opacity: 0` et ne sont révélées que par IntersectionObserver : sans JavaScript, les sections sont invisibles. Le texte reste présent dans le HTML rendu (indexation préservée), mais l'exigence « site utilisable sans JS » ne l'est pas. Ajouter une règle `<noscript>` qui neutralise les classes. | 30min | open |
